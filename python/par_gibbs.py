...
def learn(self, sweeps, step, var_copy=0, weight_copy=0):
    for sweep in range(sweeps):
        for var_samp in range(self.variable.shape[0]):
            sample_and_sgd(self, var_samp, step, var_copy, weight_copy)
        print(sweep + 1)
        print("Weights:")
...
def gibbs(self, sweeps, var_copy=0, weight_copy=0):
    ...
    for sweep in range(sweeps):
        for var_samp in range(self.variable.shape[0]):
            self.count[var_samp] += sample(self, var_samp, var_copy, weight_copy)
...
def draw_sample(self, var_samp, var_copy=0, weight_copy=0):
    ...
    for value in range(cardinality):
        self.Z[value] = math.exp(potential(self, var_samp, value, var_copy, weight_copy))
...
def sample(self, var_samp, var_copy=0, weight_copy=0):
    ...
    self.vv[var_copy][var_samp] = draw_sample(self, var_samp, var_copy, weight_copy)
    return self.vv[var_copy][var_samp]
    ...
    if self.variable[var_samp]["isEvidence"] == 1:
        for i in range(self.vstart[var_samp], self.vstart[var_samp + 1]):
            ...
            if not self.weight[weight_id]["isFixed"]:
                ...
                p0 = eval_factor(self, factor_id, var_samp, self.variable[var_samp]["initialValue"], var_copy)
                p1 = eval_factor(self, factor_id, var_samp, self.vv[var_copy][var_samp], var_copy)
...
def potential(self, var_samp, value, var_copy=0, weight_copy=0):
    ...
    for k in range(self.vstart[var_samp], self.vstart[var_samp + 1]):
        ...
        p += self.wv[weight_copy][self.factor[self.vmap[k]]["weightId"]] \
           * eval_factor(self, factor_id, var_samp, value, var_copy)
    return p
...
def gibbs_p(fg, sweeps):
    #t1 = threading.Thread(target=fg.gibbs, kwargs={"sweeps": sweeps,
    #                                               "var_copy": 0,
    #                                               "weight_copy": 0})
    t1 = threading.Thread(target=gibbs, args=[fg, sweeps, 0, 0])
    t2 = threading.Thread(target=gibbs, args=[fg, sweeps, 0, 0])
    t1.start()
    t2.start()
    t1.join()
    t2.join()
...
def main(argv=None):
    ...
    res = learn(fg, arg.learn, 0.0001, 0, 0)
    #res = gibbs(fg, arg.inference, 0, 0)
    #res = gibbs(fg, arg.inference)
    res = gibbs_p(fg, arg.inference)
